...
const logos = ["Facebook", "Instagram", "WhatsApp", "bKash", "Nagad", "Fiverr", "Upwork", "Freelancer", "Shopify", "WordPress", "Webflow", "Wix", "Vercel"]
const workflows = ["CREATE", "FILL", "CALCULATE", "CUSTOMIZE", "SAVE", "DOWNLOAD", "SHARE", "REUSE"]
const documentTypes = ["INVOICE", "RECEIPT", "PAYMENT CONFIRMATION", "QUOTATION", "ORDER CONFIRMATION", "DELIVERY NOTE", "ESTIMATE"]
const bangladeshTags = ["BDT", "bKash", "Nagad", "COD", "PAID", "PARTIALLY PAID", "DUE", "TRANSACTION ID"]
const valueTags = ["PROFESSIONAL DOCUMENTS", "FAST CREATION", "LIVE PREVIEW", "PDF READY", "EASY SHARING", "SAVE & REUSE", "BUILT FOR SMALL BUSINESSES"]
...
export function ProductShowcase() {
  ...
  return (
    <section className="product-showcase" aria-label="How Kagoj helps you work">
      ...
      <div className="showcase-marquee dark-marquee">{[...workflows, ...workflows].map((item, index) => <span key={`${item}-${index}`}>{item}<b>✦</b></span>)}</div>
      <div className="showcase-marquee document-marquee">{[...documentTypes, ...documentTypes].map((item, index) => <span key={`${item}-${index}`}>{item}</span>)}</div>
      <div className="showcase-marquee country-marquee">{[...bangladeshTags, ...bangladeshTags].map((item, index) => <span key={`${item}-${index}`}>{item}<b>|</b></span>)}</div><div className="showcase-marquee value-marquee">{[...valueTags, ...valueTags].map((item, index) => <span key={`${item}-${index}`}>{item}<b>|</b></span>)}</div>
      <style jsx>{`
        .product-showcase{overflow:hidden;margin:48px -24px 0;padding:0 24px;background:linear-gradient(180deg,transparent 0,#f8f6f1 8%,#f8f6f1 92%,transparent)}
        ...
        .showcase-marquee{gap:24px;padding:15px 0;margin-top:40px;animation:showcase-scroll 38s linear infinite;font:10px var(--font-mono);letter-spacing:.1em}.showcase-marquee span{display:flex;gap:24px;align-items:center}.showcase-marquee b{color:#98a9a2;font-weight:400}.dark-marquee{margin-inline:-24px;padding-inline:24px;background:#152c2c;color:#e3f1ea;letter-spacing:.14em}.document-marquee{margin-top:0;color:#687771}.country-marquee{margin-top:0;color:#4c7065;font-size:9px;letter-spacing:.12em;animation-duration:30s}.value-marquee{margin-top:0;color:#82908a;animation-duration:42s}
