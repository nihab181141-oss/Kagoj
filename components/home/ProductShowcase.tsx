...
const logos = ["Google Workspace", "Notion", "Stripe", "Canva", "Shopify", "QuickBooks", "Calendly"]
const workflows = ["CREATE", "CUSTOMIZE", "CALCULATE", "SAVE", "SEND", "TRACK"]
const documentTypes = ["INVOICE", "RECEIPT", "PROPOSAL", "QUOTE", "CONTRACT", "PAYMENT CONFIRMATION"]
const countries = ["Built for Bangladesh", "BDT-ready", "Made for local business", "Built for Bangladesh", "BDT-ready", "Made for local business"]
  ...
  return (
    <section className="product-showcase" aria-label="How Kagoj helps you work">
      <div className="showcase-logos" aria-label="Works with your stack">
        {[...logos, ...logos].map((logo, index) => <span key={`${logo}-${index}`}>{logo}<i>·</i></span>)}
      </div>
      ...
      <div className="showcase-marquee country-marquee">{countries.map((item, index) => <span key={`${item}-${index}`}>{item}<b>+</b></span>)}</div>
      <style jsx>{`
        ...
        .showcase-logos{margin:0 auto 42px;gap:34px;animation:showcase-scroll 30s linear infinite;color:#737878;font:10px var(--font-mono);letter-spacing:.08em;text-transform:uppercase}
        .showcase-logos span{display:flex;align-items:center;gap:34px}.showcase-logos i{color:#abb0ac;font-style:normal}
        ...
        .showcase-marquee{gap:28px;padding:14px 0;margin-top:44px;animation:showcase-scroll 35s linear infinite;font:10px var(--font-mono);letter-spacing:.12em}.showcase-marquee span{display:flex;gap:28px;align-items:center}.showcase-marquee b{color:#63d2b0}.dark-marquee{margin-inline:-24px;padding-inline:24px;background:#152c2c;color:#e3f1ea}.document-marquee{margin-top:0;color:#6b7a74}.country-marquee{margin-top:0;color:#16896f;font-size:9px;animation-duration:24s}
