...
export function DocumentRenderer({ document, templateId = 'quiet-line', customization, compact = false }: { document: DocumentState; templateId?: TemplateId; customization?: Partial<DocumentCustomization>; compact?: boolean }) {
  ...
  return <div className={`template-renderer ${compact ? 'is-compact' : ''} template-${template.id} ${dark ? 'is-dark' : ''} renderer-density-${style.density}`} style={{ '--doc-primary': style.primaryColor, '--doc-secondary': style.secondaryColor, '--doc-text': style.textColor, '--doc-radius': `${style.borderRadius}px`, '--doc-density': density, fontFamily: style.fontFamily } as React.CSSProperties}>
    ...
    <div className="renderer-intro"><div><strong>{document.documentType.toUpperCase()}</strong><small>{formatDocumentDate(document.issueDate)}</small></div><b>{formatMoney(totals.total, document.currency)}</b></div>{document.showPaymentStamp && <div className={`payment-stamp stamp-${paymentStatus.toLowerCase()}`}>{paymentStatusLabel(paymentStatus)}</div>}
    ...
    <div className="renderer-items">{document.items.slice(0, compact ? 3 : 8).map((item, index) => <div key={item.id}><span>{style.showItemNumbers && `${index + 1}. `}{item.description || `Item ${index + 1}`}</span><span>{style.showQuantity && `${item.quantity} × `}{style.showUnitPrice && formatMoney(item.unitPrice, document.currency)}{style.showLineTotal && itemColumns > 0 && `  ${formatMoney(totals.lineTotals[index], document.currency)}`}</span></div>)}</div>
    <div className="renderer-total"><span>{totals.amountDue > 0 ? 'TOTAL DUE' : 'TOTAL'}</span><strong>{formatMoney(totals.amountDue > 0 ? totals.amountDue : totals.total, document.currency)}</strong></div>
